...
export const b2Grammar: GrammarLesson[] = [
    ...
    },
    {
        ...
        explanation: {
            native: "Reported speech (discorso indiretto) in Italian involves a change of verb tenses when the reporting verb is in the past. Direct speech uses 'ha detto:' with quotation; indirect speech uses 'ha detto che' + a clause. Tense backshift rules: present → imperfect (dice 'ho fame' → ha detto che aveva fame); passato prossimo → trapassato prossimo (dice 'ho mangiato' → ha detto che aveva mangiato); future → conditional (dice 'andrò' → ha detto che sarebbe andato). For yes/no questions, use 'ha chiesto se' + clause. For information questions, use 'ha chiesto + question word + clause'. Imperative becomes label: 'Vieni!' → ha detto di venire. Demonstratives and adverbs also shift: questo → quello, qui → lì, oggi → quel giorno, domani → il giorno dopo.",
