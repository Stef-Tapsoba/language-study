...
function BreakdownBar({ label, done, total, color }: Readonly<{
    ...
    return (
        <div className="flex items-center gap-3">
            <span className="text-sm text-text-sec w-24 shrink-0">{label}</span>
            <div className="flex-1 h-2 bg-surface-elevated rounded-full overflow-hidden">
                <div className={`h-full ${color} rounded-full transition-[width] duration-700 ease-out`} style={{ width: `${pct}%` }} />
            </div>
            <span className="text-xs text-text-sec w-12 text-right shrink-0">{done}/{total}</span>
        </div>
    )
}
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-3 gap-3">
                <div className="bg-surface-card rounded-2xl border border-border-default p-4 text-center">
                    <p className="text-2xl font-bold text-violet-600">{allReviewed}</p>
                    <p className="text-xs text-text-sec mt-1">cards reviewed</p>
                    <p className="text-xs text-text-ter">last 14 days</p>
                </div>
                <div className="bg-surface-card rounded-2xl border border-border-default p-4 text-center">
                    <p className="text-2xl font-bold text-green-600">{avgAcc}%</p>
                    <p className="text-xs text-text-sec mt-1">quiz accuracy</p>
                    <p className="text-xs text-text-ter">all activities</p>
                </div>
                <div className="bg-surface-card rounded-2xl border border-border-default p-4 text-center">
                    <p className="text-2xl font-bold text-amber-500">{streak > 0 ? `${streak}🔥` : "—"}</p>
                    <p className="text-xs text-text-sec mt-1">day streak</p>
                    <p className="text-xs text-text-ter">keep it up!</p>
                </div>
            </div>
            ...
            <div className="bg-surface-card rounded-2xl border border-border-default p-4">
                <div className="flex items-center justify-between mb-3">
                    <p className="text-xs font-semibold text-text-ter uppercase tracking-wide">Daily reviews</p>
                    <div className="flex items-center gap-3 text-xs text-text-ter">
            ...
            {/* Progress breakdown */}
            <div className="bg-surface-card rounded-2xl border border-border-default p-4">
                <p className="text-xs font-semibold text-text-ter uppercase tracking-wide mb-4">Progress breakdown</p>
            ...
            {total === 0 && (
                <p className="text-center text-text-ter text-sm py-2">
